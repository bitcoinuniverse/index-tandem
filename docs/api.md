...
Pipeline A protocol routes are under `/v1/tandem`.
...
The explorer-facing contract is a separate, fail-closed surface under `/v1/tandem/verified`.

| Route | Purpose |
| --- | --- |
| `GET /v1/tandem/verified/status` | Verified deployment and readiness status |
| `GET /v1/tandem/verified/objects` | Verified object listing |
| `GET /v1/tandem/verified/objects/:key` | Verified object detail |
| `GET /v1/tandem/verified/events/:txid` | Verified canonical events for a transaction |
| `GET /v1/tandem/verified/transactions/:txid` | Verified transaction detail |
| `GET /v1/tandem/verified/addresses/:address` | Verified address activity |
| `GET /v1/tandem/verified/invalid-events` | Verified invalid-event listing |
| `GET /v1/tandem/verified/mempool` | Verified mempool overlay listing |
| `GET /v1/tandem/verified/conflicts` | Verified conflict listing |
| `GET /v1/tandem/verified/reorgs` | Verified reorg journal |
| `GET /v1/tandem/verified/stats` | Verified aggregate counts |
| `GET /v1/tandem/verified/search` | Verified explorer search |
...
Pipeline A requests pipeline B at `GET /v1/agreement/{height}`, validates each envelope against the
